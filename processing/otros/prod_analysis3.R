...
df_study1 <- df_study1_long_t7 %>%
  ...
  mutate(just_pension = as.integer(just_pension),
         just_pension = if_else(just_pension <= 3, 0, 1),
         just_pension = factor(just_pension, 
                               levels = c(0:1), 
...
g1 <- datos.pension %>% 
  ...
  scale_fill_manual(values =  c("#7e0014", "#cc4a12")) +
...
m0 <- glmer(just_pension ~ 1 + (1 | idencuesta), 
            data = df_study1, 
            family = binomial(link = "logit"))

performance::icc(m0, by_group = T) # 0.33 es between, 0.67 within

## Time effects

m1.1 <- glmer(just_pension ~ 1 + ola + (1 | idencuesta),
              data = df_study1, 
              family = binomial(link = "logit"))

m1.2 <- glmer(just_pension ~ 1 + ola_num + (1 | idencuesta),
             data = df_study1, 
             family = binomial(link = "logit"))

m1.3 <- glmer(just_pension ~ 1 + ola_num + ola_2 + (1| idencuesta),
             data = df_study1, 
             family = binomial(link = "logit"))

m1.4 <- glmer(just_pension ~ 1 + ola_num + (1 + ola_num | idencuesta),
             data = df_study1, 
             family = binomial(link = "logit"))
...
anova(m1.2, m1.4) # quedarse con tiempo continua y con pendiente aleatoria
