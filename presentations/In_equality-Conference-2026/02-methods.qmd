...
Based on previous research [@castillo_perceptions_2025; @lindh_public_2015], we controlled:

- Sociodemographic:
  
    * Sex (1 = male, 2 = female)
    * Age (in years)
    * Educational attainment (1 = less than university, 2 = university)

- Attitudinal:

    * Political identification (1 = left, 2 = center, 3 = right, 4 = no identification)
...
::: {.fragment}
$$\begin{aligned}
\eta_{it} = \beta_{0}+ \beta_{1}\,\text{Time}_{it} +\beta_2\,\text{Meritocracy}^{WE}_{it} +\beta_3\,{Meritocracy}^{BE}_{i} +\beta_4\,\mathrm{Class}_i +
    \end{aligned}$$
    
$$\beta_5\,(\mathrm{Class}_i\!\times\! \text{Meritocracy}^{WE}_{it}) +\beta_6\,(\mathrm{Class}_i\!\times\! \text{Meritocracy}^{BE}_{i}) +u_{0i}+u_{1i}\,\text{Time}_{it}\\$$
:::
